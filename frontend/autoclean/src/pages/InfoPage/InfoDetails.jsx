...
// For convenience, here is the mapping for action_type:
// ONE_OPTIONAL = 1, ONE_MANDATORY = 2, MANY_OPTIONAL = 3, MANY_MANDATORY = 4
...
export default function InfoDetails() {
  ...
  const fetchScanResults = async () => {
    ...
    try {
      ...
      const result = rawArray.map((item) => ({
        ...
        row: item.row,
        col: item.col,
        action_type: item.action_type, // new field to decide activation logic
  ...
  /**
   * toggleActivate now enforces the action type logic:
   * 
   * - ONE_OPTIONAL (1): When activating an action, any other active action will be turned off.
   * - ONE_MANDATORY (2): Exactly one action must be active. Toggling off the sole active action is blocked.
   * - MANY_OPTIONAL (3): No restrictions on toggling.
   * - MANY_MANDATORY (4): At least one action must be active.
   */
  const toggleActivate = async (scanIdx, actionIdx) => {
    const scanItem = scanResults[scanIdx];
    const actionType = scanItem.action_type;
    // Create a local copy of the actions
    const updatedActions = [...scanItem.actions];
    const targetAction = { ...updatedActions[actionIdx] };
    const newActivate = !targetAction.activate;

    // Process based on action_type
    if (actionType === 1) {
      // ONE_OPTIONAL: Optionally activate one function at most.
      if (newActivate) {
        // Activate target action and deactivate any other active action.
        updatedActions.forEach((act, idx) => {
          updatedActions[idx] = { ...act, activate: idx === actionIdx };
        });
      } else {
        // Toggling off is allowed
        updatedActions[actionIdx].activate = false;
      }
    } else if (actionType === 2) {
      // ONE_MANDATORY: Exactly one action must be active.
      if (newActivate) {
        // Activate target and turn others off.
        updatedActions.forEach((act, idx) => {
          updatedActions[idx] = { ...act, activate: idx === actionIdx };
        });
      } else {
        // If trying to deactivate the only active action, show an error.
        const activeCount = updatedActions.filter((act) => act.activate).length;
        if (activeCount === 1 && targetAction.activate) {
          Swal.fire("Error", "At least one action must be active for this rule.", "error");
          return;
        }
        updatedActions[actionIdx].activate = false;
      }
    } else if (actionType === 3) {
      // MANY_OPTIONAL: Toggle freely.
      updatedActions[actionIdx].activate = newActivate;
    } else if (actionType === 4) {
      // MANY_MANDATORY: At least one action must be active.
      if (!newActivate) {
        const activeCount = updatedActions.filter((act) => act.activate).length;
        if (activeCount === 1 && targetAction.activate) {
          Swal.fire("Error", "At least one action must be active for this rule.", "error");
        ...
        updatedActions[actionIdx].activate = true;
      }
    } else {
      // Fallback: just toggle state if action_type is missing
      updatedActions[actionIdx].activate = newActivate;
    }

    // Update the scanResults state with our changed actions for the scan item.
    setScanResult((prev) => {
      const updated = [...prev];
      updated[scanIdx] = { ...scanItem, actions: updatedActions };
      return updated;
    });

    // Send the updated state for each action to the backend.
    Promise.all(
      updatedActions.map((act) =>
        fetch("http://35.213.150.144:8000/api/import-scanresult-action/update/", {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
      ...
      Swal.fire("Error", err.message, "error");
      // Optionally you can revert state changes if needed.
    });

    // If jumpEnabled and an action has just been activated, perform the highlight navigation.
    // (This logic remains unchanged from your original implementation.)
    const newlyActivated =
      (actionType === 1 || actionType === 2 || actionType === 4 || actionType === 3) &&
      newActivate;
    if (newlyActivated && jumpEnabled) {
                                    ...
                                    return (
                                      <td
                                        ...
                                        className={`px-4 py-2 break-words text-left max-w-[400px] ${
                                          isHighlight
                                            ? "bg-yellow-200 border-2 border-red-500 animate-pulse"
                                            : ""
                                        }`}
                                      >
                                        {record.data[header] ?? "-"}
